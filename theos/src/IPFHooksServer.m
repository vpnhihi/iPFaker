// Client mitigations for server-facing surfaces (NOT a server bypass).
// - Force CFNetwork proxy when EnableProxy
// - Soft-disable App Attest / DeviceCheck client paths
// - Rewrite WebRTC ICE candidate host IP → private WebRTCLocalIP
// Linked into iPFakerCT to keep iPFakerMG Extra under Dopamine AMFI size budget.
...
static void IPFSvTrace(NSString *s) {
    @try {
        ...
        // Share Extra log so surface/holes scripts still see Proxy/AA/WebRTC lines
...
#pragma mark - WebRTC ICE candidate IP
...
static NSString *stub_iceCandidate(id self, SEL _cmd) {
    ...
    if ([[IPFConfig shared] flag:@"DisableWebRTC" defaultYes:NO]) {
        IPFSvTrace(@"WebRTC candidate blocked (DisableWebRTC)");
        return @"";
    }
    ...
    if (![out isEqualToString:real])
        IPFSvTrace([NSString stringWithFormat:@"WebRTC candidate IP → %@", fakeIP]);
    return out;
}
...
static NSDictionary *IPFProxyDictionary(void) {
    ...
    if ([type containsString:@"SOCKS"]) {
        ...
        // HTTPS* CF constants unavailable on iOS SDK — string keys (Apple CFNetwork)
...
#pragma mark - App Attest / DeviceCheck (client soft-disable only)
...
static void stub_attestAssertion(id self, SEL _cmd, id keyId, id hash, id completion) {
    if (IPFDisableAppAttest()) {
        IPFSvTrace(@"AppAttest generateAssertion blocked");
        if (completion) {
            void (^cb)(id, id) = completion;
            NSError *err = [NSError errorWithDomain:@"DCErrorDomain" code:2
                                           userInfo:@{ NSLocalizedDescriptionKey: @"App Attest disabled (iPFaker lab)" }];
...
static BOOL stub_attestIsSupported(id self, SEL _cmd) {
    if (IPFDisableAppAttest()) {
        IPFSvTrace(@"AppAttest isSupported → NO (lab)");
        return NO;
    }
...
static void stub_dcGenerateToken(id self, SEL _cmd, id completion) {
    if (IPFDisableAppAttest()) {
        IPFSvTrace(@"DeviceCheck generateToken blocked (lab)");
        if (completion) {
            void (^cb)(id, id) = completion;
            NSError *err = [NSError errorWithDomain:@"DCErrorDomain" code:2
...
void IPFInstallServerHooks(void) {
    ...
    // WebRTC ICE
    const char *rtcClasses[] = { "RTCIceCandidate", "RTC_OBJC_TYPE(RTCIceCandidate)", NULL };
    for (int i = 0; rtcClasses[i]; i++) {
        ...
        SEL cand = NSSelectorFromString(@"candidate");
        if (class_getInstanceMethod(rc, cand))
            pMSHookMessageExSV(rc, cand, (IMP)stub_iceCandidate, (IMP *)&orig_iceCandidate);
        IPFSvTrace([NSString stringWithFormat:@"RTCIceCandidate hook on %s", rtcClasses[i]]);
        break;
    }
    ...
    if (usc) {
        ...
        IPFSvTrace(@"NSURLSessionConfiguration proxy hooks OK");
    }
    ...
    if (dcd) {
        ...
        IPFSvTrace(@"DCDevice DeviceCheck hooks OK");
    }

    // Module cover line
    @try {
        NSString *row =
            @"MODULE IPFHooksServer: Proxy CFNetwork · AppAttest/DeviceCheck soft-disable · WebRTC private IP\n"
            @"SURFACE Server mitigations: client-only (Graph/OTP/public IP still need proxy+behavior)\n";
        NSString *home = NSHomeDirectory();
        if (home.length) {
